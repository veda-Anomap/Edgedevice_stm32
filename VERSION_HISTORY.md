...
변경 전 상태/문제
- GCC-PHAT lag 탐색이 고정 범위(`±8`)라 실제 마이크 간격(120mm)에서 물리적으로 불가능한 지연까지 후보에 포함되어,
  반향/잡음 상황에서 가짜 피크를 선택할 여지가 있었음.
- 프레임 경계 불연속으로 인한 스펙트럼 누설이 피크 선명도를 떨어뜨릴 수 있었음.

왜 바꿨는지
- TDOA 추정을 실제 하드웨어 기하(마이크 간격, 음속, 샘플레이트)에 맞춘 유효 범위로 제한해 오탐을 줄이기 위함.
- PHAT 이전 스펙트럼 안정화를 통해 피크 분리를 개선하기 위함.

무엇을 어떻게 바꿨는지
- `Core/Src/mic.c`
  - `tdoa_effective_lag_max()` 추가:
    - `lag_max = round(fs*d/c) + margin` 계산 후 `TDOA_LAG_MAX`로 clamp
    - lag 스캔/보간 경계를 동적으로 `±lag_max`로 사용
  - Hann window 추가:
    - `tdoa_prepare_hann()`로 128프레임 윈도우 1회 생성
    - `tdoa_dft_real_128()`에 window 인자 추가 후 DFT 입력에 적용
  - 리셋 시 Hann 준비 상태 초기화
- `Core/Src/app.c`
  - UART1 복구 함수 내 깨진 주석 정리(동작 변경 없음)

변경 후 기능/안정성 개선 효과
- 물리적으로 불가능한 lag 후보가 제거되어 반향 환경의 잘못된 방향 선택 확률 감소.
- 윈도우 적용으로 스펙트럼 누설이 줄어 피크 대조도가 개선되고 confidence 변동 완화.
- 기존 Stage 4~6(히스테리시스/EMA/추종기)와 결합 시 방향 추종 안정성 추가 향상.
